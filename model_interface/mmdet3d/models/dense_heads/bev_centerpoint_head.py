...
@HEADS.register_module()
class Centerness_Head2D(BaseModule):
    """CenterHead for CenterPoint.
    ...
    Args:
        ...
        loss_cls (dict, optional): Config of classification loss function.
            Default: dict(type='GaussianFocalLoss', reduction='mean').
        loss_slot (dict, optional): Config of regression loss function.
            Default: dict(type='L1Loss', reduction='none').
        separate_head (dict, optional): Config of separate head. Default: dict(
            type='SeparateHead', init_bias=-2.19, final_kernel=3)
        share_conv_channel (int, optional): Output channels for share_conv
    ...
    def __init__(self,
        ...
        assert init_cfg is None, 'To prevent abnormal initialization ' \
            'behavior, init_cfg is not allowed to be set'
        super(Centerness_Head2D, self).__init__(init_cfg=init_cfg)
        ...
        self.loss_cls = build_loss(loss_cls)
        self.loss_slot = build_loss(loss_slot)
        self.bbox_coder = build_bbox_coder(bbox_coder)
        self.num_anchor_per_locs = [n for n in num_classes]
        self.fp16_enabled = False
        ...
        self.task_specific_weight = task_specific_weight # 'ctr_offset', 'availability', 'kp0', 'kp1', 'kp2', 'kp3'

    def forward_single(self, x):
        """Forward function for CenterPoint.

        Args:
            x (torch.Tensor): Input feature map with the shape of [B, in_channels, H, W].
        ...
        # dict: {
        #   ctr_offset: (B, 2, H, W)
        #   availability: (B, 3, H, W) ['vacant', 'vehicle-occupied', 'other-occupied']
        #   kp0: (B, 2, H, W)
        #   kp1: (B, 2, H, W)
        #   kp2: (B, 2, H, W)
        #   kp3: (B, 2, H, W)
        #   heatmap: (B, n_cls, H, W) ['perpendicular', 'parallel', 'other']
        # }
    ...
    def get_targets(self, pl_cat, pl_sts, pl_geom):
        """Generate targets.
        ...
        """
        heatmaps, anno_cts, anno_kps, anno_sts, inds, masks = multi_apply(self.get_targets_single,
                                                                          pl_cat, pl_sts, pl_geom)

        heatmaps = list(map(list, zip(*heatmaps)))
        heatmaps = [torch.stack(hms_) for hms_ in heatmaps]

        anno_cts = list(map(list, zip(*anno_cts)))
        anno_cts = [torch.stack(anno_cts_) for anno_cts_ in anno_cts]

        anno_kps = list(map(list, zip(*anno_kps)))
        anno_kps = [torch.stack(anno_kps_) for anno_kps_ in anno_kps]

        anno_sts = list(map(list, zip(*anno_sts)))
        anno_sts = [torch.stack(anno_sts_) for anno_sts_ in anno_sts]
        ...
        return heatmaps, anno_cts, anno_kps, anno_sts, inds, masks

    def get_targets_single(self, pl_cat, pl_sts, pl_geom):
        """Generate training targets for a single sample.
        Returns:
            tuple[list[torch.Tensor]]: Tuple of target including
                the following results in order.
                ...
                - masks: list[torch.Tensor]: Masks indicating which boxes
                        are valid.              # List[(max_objs, ), (max_objs, ), ...]
        """
        device = pl_cat.device
        gt_labels = pl_cat
        gt_status = pl_sts
        gt_keypoints = pl_geom

        max_slots = self.train_cfg['max_objs'] * self.train_cfg['dense_reg']
        ...
        task_masks = []
        flag = 0
        for class_name in self.class_names: # 如果len(tasks) == 1, 这个循环只执行一次.
            # class_name: 不同task(SeparateHead)负责检测的类别名.
            task_masks.append([
                torch.where(gt_labels == class_name.index(i) + flag)
                for i in class_name
            ])
            flag += len(class_name)
        ...
        for idx, mask in enumerate(task_masks):
            ...
            task_status = []
            for m in mask:
                task_slot.append(gt_keypoints[m])
                # 0 is background for each task, so we need to add 1 here.
                task_class.append(gt_labels[m] + 1 - flag2)
                task_status.append(gt_status[m] + 1 - flag2)
            task_slots.append(torch.cat(task_slot, axis=0).to(device))
            task_classes.append(torch.cat(task_class).long().to(device))
            task_statuses.append(torch.cat(task_status).long().to(device))
            flag2 += len(mask)

        draw_gaussian = draw_heatmap_gaussian
        heatmaps, anno_cts, anno_kps, anno_sts, inds, masks = [], [], [], [], [], []

        for idx, task_head in enumerate(self.task_heads):
            heatmap = gt_keypoints.new_zeros((len(self.class_names[idx]), feature_map_size[1], feature_map_size[0]))
            # (N_cls, H, W), above line has nothing to do with keypoints, tensor.new_zeros is simply type casting

            anno_ct = gt_keypoints.new_zeros((max_slots, 2), dtype=torch.float32)  # 2: offset for center_x, center_y
            anno_kp = gt_keypoints.new_zeros((max_slots, 8), dtype=torch.float32)  # 8: kp0xy, kp1xy, kp2xy, kp3xy
            anno_st = gt_keypoints.new_zeros((3, feature_map_size[1], feature_map_size[0])) # 3 is hardcoded for now

            ind = gt_labels.new_zeros((max_slots, ), dtype=torch.int64)
            mask = gt_labels.new_zeros((max_slots, ), dtype=torch.uint8)

            num_objs = min(task_slots[idx].shape[0], max_slots)

            cur_task_slots = task_slots[idx]
            cur_task_classes = task_classes[idx]
            cur_task_statuses = task_statuses[idx]

            for k in range(num_objs):
                cls_id = task_classes[idx][k] - 1
                status_id = task_statuses[idx][k] - 1

                x_min = cur_task_slots[k, 0::2].min().item()
                x_max = cur_task_slots[k, 0::2].max().item()
                y_min = cur_task_slots[k, 1::2].min().item()
                y_max = cur_task_slots[k, 1::2].max().item()

                # step1: calculate the gaussian radius for each parking spot
                cur_slot_width = x_max - x_min
                cur_slot_height = y_max - y_min
                # to feature map
                cur_slot_width = cur_slot_width / voxel_size[0] / self.train_cfg['out_size_factor']
                cur_slot_height = cur_slot_height / voxel_size[1] / self.train_cfg['out_size_factor']
                if cur_slot_width <= 0 or cur_slot_height <= 0:
                    continue
                radius = gaussian_radius((cur_slot_height, cur_slot_width),
                                         min_overlap=self.train_cfg['gaussian_overlap'])
                radius = max(self.train_cfg['min_radius'], int(radius))

                # step2: locate the center of the parking spot
                cur_slot_ctx = torch.mean(cur_task_slots[k, 0::2])
                cur_slot_cty = torch.mean(cur_task_slots[k, 1::2])
                # to feature map coor_x are pointing right, coor_y are pointing down, located on the top-left corner
                coor_x = (cur_slot_cty - pc_range[1]) / voxel_size[1] / self.train_cfg['out_size_factor']
                coor_y = (cur_slot_ctx - pc_range[0]) / voxel_size[0] / self.train_cfg['out_size_factor']
                center = torch.tensor([coor_x, coor_y], dtype=torch.float32, device=device)
                center_int = center.to(torch.int32)
                # prune the ones not in range
                # feature_map_size[0]: width of the feature map, i.e. along with coor_x
                # feature_map_size[1]: height of the feature map, i.e. along with coor_y
                if not (0 <= center_int[0] < feature_map_size[0] and 0 <= center_int[1] < feature_map_size[1]):
                    continue

                # step3: draw the gaussian heatmap & status heatmap
                draw_gaussian(heatmap[cls_id], center_int, radius)
                draw_gaussian(anno_st[status_id], center_int, radius)

                # step4: record the center point flattened indices and mask (for valid spots)
                x, y = center_int[0], center_int[1]
                assert (y * feature_map_size[0] + x) < (feature_map_size[0] * feature_map_size[1])
                ind[k] = y * feature_map_size[0] + x
                mask[k] = 1

                # step5: format the regressions for center offset
                anno_ct[k] = center - center_int

                # step6: format the regressions for keypoints
                # to featuremap
                kpsx = (cur_task_slots[k, 1::2] - pc_range[1]) / voxel_size[1] / self.train_cfg['out_size_factor']
                kpsy = (cur_task_slots[k, 0::2] - pc_range[0]) / voxel_size[0] / self.train_cfg['out_size_factor']
                kpsx_offset = (kpsx - center_int[0])
                kpsy_offset = (kpsy - center_int[1])
                anno_kp[k] = torch.cat((kpsx_offset, kpsy_offset)) # 8: kp0123_x, kp0123_y

            ##############
            # import numpy as np
            # hm = heatmap[0,:,:].squeeze(0).cpu().numpy()
            # hm_max = hm.max()
            # hm = (hm / hm_max * 255).astype(np.uint8)

            # import cv2
            # cv2.imwrite('parkinglot_hm.png', hm)
            ##############

            heatmaps.append(heatmap)
            anno_cts.append(anno_ct)
            anno_kps.append(anno_kp)
            anno_sts.append(anno_st)
            inds.append(ind)
            masks.append(mask)
        return heatmaps, anno_cts, anno_kps, anno_sts, inds, masks

    def loss(self, preds_dicts, pl_cat, pl_sts, pl_geom):
        """Loss function for CenterHead.
        Returns:
            dict[str:torch.Tensor]: Loss of heatmap and bbox of each task.
        """
        heatmaps, anno_cts, anno_kps, anno_sts, inds, masks = self.get_targets(pl_cat, pl_sts, pl_geom)
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            # dict: {
            #   ctr_offset: (B, 2, H, W)
            #   availability(status): (B, 3, H, W) ['vacant', 'vehicle-occupied', 'other-occupied']
            #   kp0: (B, 2, H, W)
            #   kp1: (B, 2, H, W)
            #   kp2: (B, 2, H, W)
            #   kp3: (B, 2, H, W)
            ...
            # availability status focal loss
            preds_dict[0]['availability'] = clip_sigmoid(preds_dict[0]['availability'])
            num_pos = anno_sts[task_id].eq(1).float().sum().item()
            cls_avg_factor = torch.clamp(
                reduce_mean(anno_sts[task_id].new_tensor(num_pos)),
                min=1).item()
            loss_status = self.loss_cls(
                preds_dict[0]['availability'],   # (B, 3, H, W)
                anno_sts[task_id],          # (B, 3, H, W)
                avg_factor=cls_avg_factor
            )

            target_ctr_offset = anno_cts[task_id]
            target_kps = anno_kps[task_id]
            target_parkingspot = torch.cat(
                (
                    target_ctr_offset,  # (B, max_slots, 2) offset from hm center to the parking spot center
                    target_kps[:, :, 0::4],   # (B, max_slots, 2) kp0 offset from hm center to the parking spot corner
                    target_kps[:, :, 1::4],   # (B, max_slots, 2) kp1 offset from hm center to the parking spot corner
                    target_kps[:, :, 2::4],   # (B, max_slots, 2) kp2 offset from hm center to the parking spot corner
                    target_kps[:, :, 3::4],   # (B, max_slots, 2) kp3 offset from hm center to the parking spot corner
                ),
                dim=2,
                # (B, max_slots, 10) 10: (ctx, cty, kp0x, kp0y, kp1x, kp1y, kp2x, kp2y, kp3x, kp3y)
            )

            pred_parkingspot = torch.cat(
                (
                    preds_dict[0]['ctr_offset'],
                    preds_dict[0]['kp0'],
                    preds_dict[0]['kp1'],
                    preds_dict[0]['kp2'],
                    preds_dict[0]['kp3'],
                ...
                dim=1,
                # (B, 10, H, W) 10: (ctx, cty, kp0x, kp0y, kp1x, kp1y, kp2x, kp2y, kp3x, kp3y)
            )

            num = masks[task_id].float().sum()     # 正样本的数量
            ind = inds[task_id]     # (B, max_objs)
            pred = pred_parkingspot.permute(0, 2, 3, 1).contiguous()    # (B, H, W, 10)
            pred = pred.view(pred.size(0), -1, pred.size(3))            # (B, H*W, 10)
            pred = self._gather_feat(pred, ind)                         # (B, max_objs, 10)
            mask = masks[task_id].unsqueeze(2).expand_as(target_parkingspot).float()
            num = torch.clamp(
                reduce_mean(target_parkingspot.new_tensor(num)), min=1e-4).item()
            isnotnan = (~torch.isnan(target_parkingspot)).float()
            mask *= isnotnan    # 只监督mask指定的reg预测.
            code_weights = self.train_cfg['code_weights']
            slots_weights = mask * mask.new_tensor(code_weights)   # 在mask基础上，设置不同regression权重 (B, max_objs, 10)
            if self.task_specific:
                name_list = ['ctr_offset', 'kp0', 'kp1', 'kp2', 'kp3']
                clip_index = [0, 2, 4, 6, 8, 10]
                for reg_task_id in range(len(name_list)):
                    pred_tmp = pred[..., clip_index[reg_task_id]:clip_index[reg_task_id + 1]]
                    target_parkingspot_tmp = target_parkingspot[..., clip_index[reg_task_id]:clip_index[reg_task_id + 1]]
                    slots_weights_tmp = slots_weights[..., clip_index[reg_task_id]:clip_index[reg_task_id + 1]]
                    loss_slots_tmp = self.loss_slot(
                        pred_tmp,
                        target_parkingspot_tmp,
                        slots_weights_tmp,
                        avg_factor=(num + 1e-4))
                    loss_dict[f'task{task_id}.loss_%s' %
                                (name_list[reg_task_id])] = loss_slots_tmp * self.task_specific_weight[reg_task_id]
                loss_dict[f'task{task_id}.loss_heatmap'] = loss_heatmap
                loss_dict[f'task{task_id}.loss_status'] = loss_status
            else:
                loss_slots = self.loss_slot(
                    pred, target_parkingspot, slots_weights, avg_factor=num)
                loss_dict['loss'] += loss_slots
                loss_dict['loss'] += loss_heatmap
                loss_dict['loss'] += loss_status
    ...
    def get_bboxes(self, preds_dicts, img_metas, img=None, rescale=False):
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            batch_size = preds_dict[0]['heatmap'].shape[0]
            batch_heatmap = preds_dict[0]['heatmap'].sigmoid()      # (B, n_cls, H, W)
            batch_status = preds_dict[0]['availability'].sigmoid()  # (B, 3, H, W)

            batch_ctr_reg = preds_dict[0]['ctr_offset']              # (B, 2, H, W)
            batch_kp0 = preds_dict[0]['kp0']
            batch_kp1 = preds_dict[0]['kp1']
            batch_kp2 = preds_dict[0]['kp2']
            batch_kp3 = preds_dict[0]['kp3']
            ...
            batch_centers = [box['centers'] for box in temp]    # List[(K, 2), (K, 2), ...]   len = bs
            batch_scores = [box['scores'] for box in temp]      # List[(K, ), (K, ), ...]   len = bs
            batch_labels = [box['labels'] for box in temp]      # List[(K, ), (K, ), ...]   len = bs
            batch_status = [box['status'] for box in temp]      # List[(K, ), (K, ), ...]   len = bs
            batch_kps = [box['kps'] for box in temp]            # List[List((K, 2), ...), ...] len = bs

            rets = [batch_centers, batch_scores, batch_labels, batch_status, batch_kps]
